...
def fetch_user_information():
    ...
    cursor = conn.cursor()
    query = "SELECT username, nameWebsite, website, status FROM WebsiteBuilder"
...
def update_user_status(nameWebsite, db_port, app_port):
    ...
    cursor = conn.cursor()
    query = "UPDATE WebsiteBuilder SET status = true, db_port = %s, app_port = %s WHERE nameWebsite = %s"
