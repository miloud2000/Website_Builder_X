...
def update_user_status(username, db_port, app_port):
    # Connect to the MySQL database
    conn = mysql.connector.connect(
        host="localhost",  # MySQL container hostname
        user="root",
    ...
    cursor = conn.cursor()
    query = "UPDATE websites SET status = true, db_port = %s, app_port = %s WHERE username = %s"
    cursor.execute(query, (db_port, app_port, username))
...
def create_docker_containers(users):
 ...
 # Find an available port for the website container
            ...
            # Update user status
            update_user_status(username,db_port,app_port)
            print(f"Container for user {username} created successfully --> app_port:{app_port} --> db_port:{db_port} ")
