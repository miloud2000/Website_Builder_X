...
def create_docker_containers(users):
    for user in users:
        ...
        if not status:  # If status is false
            ...
            # Create database container with volume mounted
            subprocess.run(["docker", "run", "-d", "--name", db_container_name, "-p", f"{db_port}:3306", "-e", "MYSQL_ROOT_PASSWORD=password", "-v", f"/mysql-volume/{username}:/var/lib/mysql", "mysql:latest"])

            # Create website container with volume mounted
            subprocess.run(["docker", "run", "-d", "--name", app_container_name, "-p", f"{app_port}:80", "-v", f"/wordpress-volume/{username}:/var/www/html", base_image])
