...
def fetch_user_information():
    ...
    cursor = conn.cursor()
    query = "SELECT username, nameWebsite, website, status FROM LocationWebsiteBuilder"
...
def update_user_status(nameWebsite, db_port, app_port):
    ...
    cursor = conn.cursor()
    query = "UPDATE LocationWebsiteBuilder SET status = true, db_port = %s, app_port = %s WHERE nameWebsite = %s"
